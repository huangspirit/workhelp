<template>
	<view class="addbusketDetail">
		...
		<view class="detail">
			...
			<view  class="item">
			<text class="gray">库存：{{choseGuige.showCount>choseGuige.num ? choseGuige.num:"库存充足"}}</text>
					...
					for(let i=0;i<arr.length;i++){
						...
						if(item.id==this.goodsDetail.id) {
							...
							if(item.choseGuige.id == this.choseGuige.id){
								...
								item.goodsNumber = count >this.choseGuige.singleCount?this.choseGuige.singleCount:count;
								busket[i] = item
							}else{
